<template>
    <div>
        <ol>
            ...
            <li :class="index === SelectedProduct ? 'ActiveProduct':''"  @click="SelectGroup(index)" v-for="Product,index in Products" :key="index">
...
<li  v-for="SubProduct,index in Product.subgroups" :key="index">
    <router-link :to="`/product/${SubProduct.id}`">{{ SubProduct.title }}
    </router-link> </li>
    </ul>
    </li>
